...
export class BvHttpError extends Error {
  constructor(
    ...
    public status: number = CtraficHttpStatusCode.INTERNAL_SERVER_ERROR,
    public error?: Error
  ) {
    super();
  }
  static throwError<T = any>(
    ...
    code = CtraficHttpStatusCode.INTERNAL_SERVER_ERROR
  ) {
    if (!data) {
  ...
  //          } else {
  //             const errorInstance = new BvHttpError(error.message, CtraficHttpStatusCode.INTERNAL_SERVER_ERROR, error);
